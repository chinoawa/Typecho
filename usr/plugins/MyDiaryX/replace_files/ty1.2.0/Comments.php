...
class Comments extends Base implements QueryInterface
{
    ...
    public function insert(array $rows): int
    {
        ...
        $insertStruct = [
            ...
            'weather' => !isset($rows['weather']) || strlen($rows['weather']) === 0 ? null : $rows['weather'],
            'temperature' => !isset($rows['temperature']) || strlen($rows['temperature']) === 0 ? null : $rows['temperature'],
            'secert' => !isset($rows['secert']) || strlen($rows['secert']) === 0 ? null : $rows['secert'],
            'mood' => !isset($rows['mood']) || strlen($rows['mood']) === 0 ? null : $rows['mood'],
            'image'  => !isset($rows['image']) || strlen($rows['image']) === 0 ? null : $rows['image'],
    ...
    public function select(): Query
    {
        return $this->db->select(
            ...
            'table.comments.mail',
            'table.comments.url',
            'table.comments.weather',
            'table.comments.temperature',
            'table.comments.secert',
            'table.comments.mood',
            'table.comments.image',
            'table.comments.ip',
